...
def job(
    ...
    param_grid = hyperparameters(random_state, do_lcs_sweep, nu, iterations, N)[
        algorithm
    ]
    runModel(
        algorithm,
...
def hyper_eval(est, x_train, y_train, randSeed, hype_cv, params, scoring_metric):
    ...
    performance = np.mean(
        cross_val_score(model, x_train, y_train, cv=cv, scoring=scoring_metric)
    )
    return performance
    ...
    if not isSingle:  # Run hyperparameter sweep
        ...
        study.optimize(
            lambda trial: objective_LR(
                trial, est, x_train, y_train, randSeed, 3, param_grid, primary_metric
            ),
            n_trials=n_trials,
            timeout=timeout,
            catch=(ValueError,),
...
def objective_XGB(
    ...
    params = {
        ...
        "num_class": trial.suggest_categorical("num_class", param_grid["num_class"]),
    }
    return hyper_eval(est, x_train, y_train, randSeed, hype_cv, params, scoring_metric)
...
def run_XGB_full(
    ...
    if training_subsample > 0 and training_subsample < x_train.shape[0]:
        sss = StratifiedShuffleSplit(
            n_splits=1, train_size=int(training_subsample), random_state=randSeed
        )
...
def objective_LGB(
    ...
    params = {
        ...
        "subsample_freq": trial.suggest_int(
            "subsample_freq", param_grid["subsample_freq"][0], param_grid["subsample_freq"][1]
        ),
...
def run_KN_full(
    ...
    if training_subsample > 0 and training_subsample < x_train.shape[0]:
        ...
        for train_index, _ in sss.split(x_train, y_train):
            ...
            y_train = y_train[train_index]
        print(
            "For KN, training sample reduced to " + str(x_train.shape[0]) + " instances"
        )
    # Specify algorithm for hyperparameter optimization
    ...
    if not isSingle:  # Run hyperparameter sweep
        ...
        if eval(do_plot):
            fig = optuna.visualization.plot_parallel_coordinate(study)
            fig.write_image(
                full_path + "/models/KN_ParamOptimization_" + str(i) + ".png"
            )
        # Print results and hyperparamter values for best hyperparameter sweep trial
        print("Best trial:")
        best_trial = study.best_trial
        ...
        clf = est.set_params(**best_trial.params)
        export_best_params(
            full_path + "/models/KN_bestparams" + str(i) + ".csv", best_trial.params
        )  # Export final model hyperparamters to csv file
    else:  # Specify hyperparameter values (no sweep)
        params = copy.deepcopy(param_grid)
        for key, value in param_grid.items():
        ...
        clf = est.set_params(**params)
        export_best_params(
            full_path + "/models/KN_usedparams" + str(i) + ".csv", params
        )  # Export final model hyperparamters to csv file
    print(clf)  # Print basic classifier info/hyperparmeters for verification
    # Train final model using whole training dataset and 'best' hyperparameters
    model = clf.fit(x_train, y_train)
...
def run_ExSTraCS_full(
    ...
    # Save model with pickle so it can be applied in the future
    pickle.dump(
        model, open(full_path + "/models/pickledModels/ExSTraCS_" + str(i), "wb")
    )
    # Prediction evaluation on hold out testing data
    y_pred = clf.predict(x_test)
    # Calculate standard classificaction metrics
